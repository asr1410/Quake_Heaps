...
void insertElement(int val)
{
    ...
    map[val] = l_node;
    count_level[val] += 1;
    forest[i_node->level].push_back(i_node);
...
void trim(Internal_Node *node)
{
    ...
    forest[left->level].push_back(left);
    forest[right->level].push_back(right);
    count_level[node->level] -= 1;
    node->level = 0;
    delete node;
}
...
void cut(Internal_Node *node)
{
    ...
    forest[node->level].push_back(node);
}
